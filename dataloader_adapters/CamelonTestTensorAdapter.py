...
class CamelonTestTensorAdapter(InputTensorAdapter):
    __slots__ = ['labels']

    def __init__(self, data_paths: tuple, label_paths: tuple, rate: int = 8, num_focus: int = 4,
    ...
    def getStageOneInputTensor(self) -> (Tensor, Tensor, Tensor):
        ...
        for i in self.data_paths:
            try:
                ...
                img256 = cv2.imread('/nfs3-p1/lhm/CAMEL/s256/test/' + code + '.png')
    ...
    def getStageTwoInputTensor(self, focus_index) -> (Tensor, Tensor, Tensor):
        ...
        for i, path in enumerate(self.data_paths):
            try:
                ...
                code = filename.split('.')[0]
                img16 = cv2.imread('/nfs3-p1/lhm/CAMEL/s16/test/' + code + '.png')
