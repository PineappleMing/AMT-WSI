...
model1.load_state_dict(torch.load(HP.models_path + 'camel1.pth'))
model2.load_state_dict(torch.load(HP.models_path + 'camel2.pth'))
model3.load_state_dict(torch.load(HP.models_path + 'camel3.pth'))
attn_clsfr1.load_state_dict(torch.load(HP.models_path + 'camel_atten1.pth'))
attn_clsfr2.load_state_dict(torch.load(HP.models_path + 'camel_atten2.pth'))
...
def train(epoch):
    ...
    for train_i, (medical_tag_path, label_path) in enumerate(train_loader):
        ...
        input_adapter = CamelonInputTensorAdapter(data_paths=medical_tag_path, label_paths=label_path,
        ...
        analyzer.saveToFile(global_step)
