...
loss_se = HP.loss_schedule(max_epoch=50)
...
def train(epoch):
    ...
    for train_i, (medical_tag_path, label_path) in enumerate(train_loader):
        ...
        if global_step > 0 and global_step % 200 == 0:
            torch.save(model1.state_dict(), HP.models_path + 'panda1.pth')
            torch.save(model2.state_dict(), HP.models_path + 'panda2.pth')
            torch.save(model3.state_dict(), HP.models_path + 'panda3.pth')
            torch.save(attn_clsfr1.state_dict(), HP.models_path + 'panda_atten1.pth')
            torch.save(attn_clsfr2.state_dict(), HP.models_path + 'panda_atten2.pth')
